...
--);
--COMMENT ON TABLE test.customer is 'musteri tablosu';
--COMMENT ON COLUMN test.customer.first_name is 'musterinin ilk adi';
--COMMENT ON COLUMN test.customer.last_name is 'musterinin soyadi';
--INSERT INTO test.customer(first_name, last_name, email, company, street, city, state, zip, phone, birth_date, sex, date_entered) 
...
--create type test.sex_type as enum ('M', 'F');

--ALTER TABLE test.customer ALTER COLUMN sex TYPE test.sex_type USING sex::test.sex_type;
...
--  type_id INTEGER REFERENCES test.product_type(id),
--  name VARCHAR(30) NOT NULL,
...
--product_id INTEGER REFERENCES test.product(id),
...
--cust_id INTEGER REFERENCES test.customer(id),
--sales_person_id INTEGER REFERENCES test.sales_person(id),
...
--item_id INTEGER REFERENCES test.item(id),
--sales_order_id INTEGER REFERENCES test.sales_order(id),
...
--ALTER TABLE test.sales_item ADD day_of_week VARCHAR(8)

--ALTER TABLE test.sales_item ALTER COLUMN day_of_week SET NOT NULL;
--ALTER TABLE test.sales_item RENAME COLUMN day_of_week TO weekday;
--ALTER TABLE test.sales_item DROP COLUMN weekday;
...
--ALTER TABLE test.transaction_type RENAME TO transaction;

--CREATE INDEX transaction_id --name of the index
--ON test.transaction(name); --which column will be indexed

-- Create an index based on 2 columns
--CREATE INDEX transaction_id_2 ON test.transaction(name, payment_type)

-- Delete data in a table
--TRUNCATE TABLE test.transaction;
 
-- Drop a table
-- DROP TABLE test.transaction;

--Inserting Data
--INSERT INTO test.product_type (name) VALUES ('Business');
--INSERT INTO test.product_type (name) VALUES ('Casual');
--INSERT INTO test.product_type (name) VALUES ('Athletic');
--select * from test.product_type;

--INSERT INTO test.product VALUES
...
--ALTER TABLE test.customer ALTER COLUMN zip TYPE INTEGER;
--
-- INSERT INTO test.customer (first_name, last_name, email, company, street, city, state, zip, phone, birth_date, sex, date_entered) VALUES 
...
--ALTER TABLE test.sales_person ALTER COLUMN zip TYPE INTEGER;
--
--INSERT INTO test.sales_person (first_name, last_name, email, street, city, state, zip, phone, birth_date, sex, date_hired) VALUES 
...
--ALTER TABLE test.sales_order ALTER COLUMN purchase_order_number TYPE BIGINT;
--INSERT INTO test.sales_order VALUES 
...
--
--select * from test.sales_order;


--Insert Sales Item
--INSERT INTO test.sales_item VALUES 
...
<> : Not Equal
!= : Not Equal*/
-- SELECT * FROM test.sales_item WHERE discount > .15;
...
--
--SELECT * FROM test.sales_item WHERE discount > .15 ORDER BY discount DESC;
-- SELECT * FROM test.sales_item WHERE discount > .15 ORDER BY discount DESC LIMIT 5;
--
--We can use CONCAT to merge to columns. We can then use AS to define a new column name.
--SELECT CONCAT(first_name,  ' ', last_name) AS Name, phone, state FROM test.customer WHERE state = 'TX';

--GROUP BY
-- SUM, AVG gibi fonksiyonlarda GROUP BY kullanmak gerekir yoksa calismaz
--SELECT product_id, SUM(price) AS Total FROM test.item WHERE product_id=1 GROUP BY product_id;
--

--Distinct
--You can use distinct to eliminate duplicates in results. Get a list of states we have customers in.
--simdi burada her bir musteri icin state bilgisi gelecek ayni state birden fazla musteride var 
--bunun tekrarini onlemek ciin DISTINCT
...
--We can get results from multiple tables with either inner joins, outer joins, or unions. 
--The most common join is the inner join. You join data from 2 tables in the FROM clause with the JOIN keyword. 
--The ON keyword is used to define the join condition. Get all items ordered ever and sort them by id while listing their price 

--Inner Join
--SELECT item_id, price
--FROM test.item 
--INNER JOIN test.sales_item
--ON item.id = sales_item.item_id
--ORDER BY item_id;

--We use the join condition to find ids that are equal in the tables item and sales_item. 
--These joins are normally done using the primary and foreign keys in the tables as we did here. 
--When we join tables while checking for equality between a common column this is called a equijoin.
...
/*Outer Joins
bunda sagdaki tablda null olanlar da listelenir
Outer joins return all of the rows from one of the tables being joined even if no matches are found.
 
A Left Outer Join returns all rows from the table being joined on the left. 
The Right Outer Join returns all rows from the table on the right. 
It's common practice to avoid Right Outer joins though.
...
--Cross Joins
--Cross joins include data from each row in both tables. I'll grab information from the item and sales_item table. 
--This will produce many results. Since there are no join conditions in a Cross Join you will rarely use them.
 
--SELECT sales_order_id, quantity, product_id
--FROM test.item CROSS JOIN test.sales_item
...
Let's say we want to send birthday cards to all customers and sales persons for the month of December we could do this. 
Always put the Order By statement last. The column names are taken from those provided in the 1st select statement. 
(We use Extract to get just the month from the birth date)
*/ 
...
--TEXT ARAMA STRING ARAMA


--SIMILAR LIKE ~ & REGEXP
--SIMILAR is used to search for simple string matches. Match any customers whose name begins with M
--mysqldeki like yerine SIMILAR TO
 
--% sifir veya daha fazla karakter
--SELECT first_name, last_name
--FROM test.customer
--WHERE first_name SIMILAR TO 'M%';

--MYSQLdekki ? yerine _ (alt cizgi ) tek bir karakter
--A ile baslayan sonra 5 harf daha olanlari getir
--SELECT first_name, last_name
--FROM test.customer
...
--simdi burada count'a gore bir sinirlama yapmak istiyorsak bunu where kisminda yapamayiz
--count query sonucunda hesaplanan birsey having query olustuktan sonra filtereleme yapiyor.
...
--FROM test.sales_order     -- Join some tables
--JOIN test.sales_item
...
--SELECT * FROM test.purchase_order_overview;
...
--hesaplamalari viewde degilde view uzerinde calistirdigimiz sql de yaparsak bu is olur


--drop VIEW test.purchase_order_overview;
